...
drivers_train <- drivers |> 
  dplyr::filter(sitename %in% splits[[1]]$train_sitename)
drivers_test <- drivers |> 
  dplyr::filter(sitename %in% splits[[1]]$test_sitename)
```

<!-- Test with parallel computation: No gains! -->
<!-- ```{r} -->
<!-- par <- list( -->
<!--   kphio = 0.04102119, -->
<!--   kphio_par_a = -0.002289344, -->
<!--   kphio_par_b = 15.25094, -->
<!--   soilm_thetastar = 157.7507, -->
<!--   soilm_betao = 0.0001169702, -->
<!--   err_gpp = 1.074112 -->
<!--   ) -->

<!-- tictoc::tic() -->
<!-- notparallel <- runread_pmodel_f( -->
<!--   drivers, -->
<!--   par, -->
<!--   makecheck = FALSE, -->
<!--   parallel = FALSE, -->
